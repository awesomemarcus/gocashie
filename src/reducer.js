	...
	switch (type) {
		...
		case 'ADD_CONTRIBUTION':
			return {
				...
				contributions: {
					...state.contributions,
					list: [payload.contribution, ...state.contributions.list],
		...
		case 'ADD_LOAN':
			return {
				...
				loans: {
					...state.loans,
					list: [payload.loan, ...state.loans.list],
		...
		case 'ADD_PAYMENT':
			return {
				...
				payments: {
					...state.payments,
					list: [payload.payment, ...state.payments.list],
				...
				loans: {
					...
					list: state.loans.list.map(loan => {
						if (loan._id === payload.loan_id) {
							...
							const status =
								terms_paid >= loan.terms ? 'accepted' : loan.status;
